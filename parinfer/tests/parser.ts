// Forked from: Parinfer Test 3.13.1 MIT License Copyright 2015-2017 © Shaun Lebron
...
export function indentMode (text, extras) {
...
export function parenMode (text, extras) {
  const parsed = parseInput(text, extras)
  const result = parinfer.parenMode(parsed.text, parsed.options)
  return printOutput(result, extras)
}

export function smartMode (text, extras) {
  const parsed = parseInput(text, extras)
